...
`TraceThread` is a wrapper for a function that just calls it once. It also names the thread and handles `boost::thread_interrupted` exception. Usually used as _fn_ argument to thread constructor `thread (Fn&& fn, Args&&... args)`.
...
`std::thread threadSocketHandler` is created using CConnman::ThreadSocketHandler() wrapped into `TraceThread(...)`.
