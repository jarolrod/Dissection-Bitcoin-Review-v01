...
The function that perform the script verification is `bool interpreter.cpp:VerifyScript(...)`. It is called in at least three points of the application:

* When the node receives a new transaction, which is handled by `void net_processing.cpp:PeerManager::ProcessMessage(...)` when the received message is a _tx_ type (`if (msg_type == NetMsgType::TX)`).

* When the node wants to broadcast a new transaction. It is done by `TransactionError transaction.cpp:BroadcastTransaction(...)`.

* When receiving a new block, `void net_processing.cpp:PeerManager::ProcessMessage(...)` will identify a _getblocks_ message type (`if (msg_type == NetMsgType::GETBLOCKS)`) and will call `bool validation.cpp:CChainState::ActivateBestChain(...)`.

In the fist two case, `bool validation.cpp:AcceptToMemoryPool(...)` function is called to handle the new transaction. It will try to add the transaction to mempool. In the last case, the function that will handle the new block is `bool CChainState::ConnectBlock(...)`. Both of them ends up calling `validation.cpp:bool CheckInputScripts(...)`.

`CheckInputScripts(...)` validates the scripts of all the inputs of the `const CTransaction& tx` transaction passed as parameter. However, the relevant parameter in this context is the `std::vector<CScriptCheck> *pvChecks = nullptr`. `CScriptCheck` is a closure representing one script verification and it stores references to the spending transaction.
...
The `CScriptCheck` method that matters is the `bool validation.cpp:CScriptCheck::operator()()`. It overloads the operator `()` and perform the script validation (`VerifyScript(...)`).
...
The only function that makes use of script validation parallelization is the aforementioned `bool CChainState::ConnectBlock(...)` due to the quantity of transactions in a block. If the `g_parallel_script_checks` global parameter is true, the script verification vector that has been filled in `CheckInputScripts(...)` is allocated in  `CCheckQueueControl<CScriptCheck> control(...)`. The `control.Wait()` initiates the their execution and wait for the end. +
`g_parallel_script_checks` is set in `AppInitMain(...)` that will be shown soon.
...
The code that will be excute the work (in this case, it is the script verification) can be found in `bool src/checkqueue.h:CCheckQueue::Loop(...)`.
...
The number of script-checking threads that will be used is defined in `init.cpp:AppInitMain(...)`. The user can set the number of the threads using the argument `-par`. If the number is negative, it will limit the threads. +
If the user does not pass the `-par` parameter, `GetNumCores()` is called to get the number of concurrent threads supported by the implementation. Then 1 is subtracted from this number because the the main thread is already being  used. `GetNumCores()` is just a wrapper for C++ standard function `std::thread::hardware_concurrency()`. +
There is also a maximum number of dedicated script-checking threads allowed, that is 15 (`MAX_SCRIPTCHECK_THREADS = 15`).
Note that `g_parallel_script_checks` is set to true, allowing parallelization in the `ConnectBlock(...)` function.
...
And finally the command `ThreadScriptCheck(i);` simply initiate a new worker thread one or several times, according to the `script_threads` value. Its implementation can be found in `src/validation.cpp`.
