...
A lot of event happens at same time in Bitcoin Core: new messages arrives all time, are processed and sometime, announcements need to be done. For example, if a wallet is connected to Bitcoin Core and a transaction related to this wallet arrives, the wallet needs to be notified; when a new block arrives, the chain and the wallet need to be updated; transaction also can be removed from mempool and it needs to be notified and so on.

A good requirement for a mechanism to handle those notifications between components is to be asynchronous since any processing cannot be blocked due to notification as response is not needed.
